...
public class KDTREE
{
  ...
  Node root;
	...
	public void insert(double latitude,double longitude,int level,double zipcode,String city,String state )
	{
		Node n1=new Node(latitude,longitude,level,zipcode,city,state);
	...
	public void search_nearest_neighbour(double latitude,double longitude,int level)
	{
		...
		System.out.println("Nearest Neighbour is:"+parIns.getlatitude()+","+parIns.getlongitude()+","+parIns.getzipcode() 			+","+parIns.getcity()+","+parIns.getstate());
		}
		...
		long starttime = System.nanoTime();
			...
			{	String fileName = "Lat_Long.csv";
				...
				for(int i=0;i<32770;i++)
				{
					...
					String city=line[3];
					String state=line[4];
					b1.insert(latitude,longitude,0,zipcode,city,state);
